...
#define MAX_CMD_VIDEO_PACKAGE_SIZE 2304000
					...
					if (cmd_video->mPackageSize > 4){
						...
						if (1) {
							...
							case 0:// video
								...
								break;
							case 1:// string
								if((cmd_video->mPackageSize - 16 == (int)len) || ((cmd_video->mPackageSize - 4 - 16 == (int)len)))
...
bool DepthCameraCmdVideo::Open(std::string & camera_name)
{
	...
	if (ret) {
		...
		while (getline(ss, line)) {
			long pos;
			if ((pos = line.find("Product")) != static_cast<long>(string::npos)) {
				dev_name = line.substr(pos + 2 + strlen("Product"));
				continue;
			}

			if ((pos = line.find("Video Width")) != static_cast<long>(string::npos)) {
				string v = line.substr(pos + 2 + strlen("Video Width"));
				mVideoWidth = std::stoi(v);
				continue;
			}

			if ((pos = line.find("Video Height")) != static_cast<long>(string::npos)) {
